...
export default async function handler(
  ...
  try {
    const { walletId, fromDate, toDate } = req.body;
    ...
    console.log("line-33", walletId, token, 2, fromDate, toDate);
    const result = await getPayments(walletId, token, 2, fromDate, toDate);
