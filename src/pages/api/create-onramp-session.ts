...
interface CustomOnrampSessionResource extends Stripe.StripeResource {
  create(params: any): Promise<OnrampSessionResponse>;
}
...
export default async function handler(
  ...
  try {
    ...
    const { transaction_details, wallet_address } = req.body;
    ...
    // Create session parameters according to Stripe's documentation
    const sessionParams = {
      transaction_details: {
        destination_exchange_amount: transaction_details.destination_exchange_amount,
      ...
      wallet_addresses: {
        ethereum: wallet_address, // Set the wallet address for Ethereum
      },
      destination_network: transaction_details.destination_network,
      destination_currency: transaction_details.destination_currency,
      customer_ip_address: req.socket.remoteAddress || '',
    ...
    const onrampSession: OnrampSessionResponse = await new OnrampSessionResource(stripe).create(sessionParams);
