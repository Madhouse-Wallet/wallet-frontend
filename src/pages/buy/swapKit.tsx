...
import { createSwapKit, SwapKitApi, Chain, WalletOption } from "@swapkit/sdk";
...
const SwapKit = () => {
  ...
  const [swapKitClient, setSwapKitClient] = useState<any>(null);
  const [wallet, setWallet] = useState<any>(null);
  const [walletRoute, setWalletRoute] = useState<any>(null);
      ...
      if (userAuth?.passkeyCred) {
        ...
        if (account) {
          ...
          if (provider) {
            ...
            setProviderr(provider?.kernelProvider);
            try {
              connectEVNWAllet(provider?.kernelProvider);
            } catch (err: any) {
  ...
  const connectEVNWAllet = (kernelProvider: any) => {
    const swapKitClient = createSwapKit({
      config: {
        stagenet: process.env.NEXT_PUBLIC_STAGENET === "true",
        covalentApiKey:
        ...
        trezorManifest: {
          ...
          appUrl: process.env.NEXT_PUBLIC_TREZOR_APP_URL || "",
        },
      },
    });
    console.log("line-53", swapKitClient);
    setSwapKitClient(swapKitClient);

    const wallet = swapKitClient?.connectEVMWallet(
      connectChains,
      WalletOption.METAMASK,
      kernelProvider
    );
    console.log("wallet", wallet);
    fetchUSDCBalance(swapKitClient, wallet);
    setWallet(wallet);
  };

  // const handleSwap = async (route: any) => {
  //   if (!wallet || !swapKitClient || !route) {
  //     return;
  //   }
  //   try {
  //     console.log("route",route,swapKitClient,wallet)
  //     const swap = await swapKitClient?.swap({
  //       route:route,
  //     });
  //     console.log(swap);
  //   } catch (error) {
  ...
  const handleSwap = async (route: any) => {
    ...
    const maxRetries = quotes.length;
    let currentIndex = currentQuoteIndex;

    while (currentIndex < maxRetries) {
      try {
        const currentRoute = quotes[currentIndex];
        console.log("Attempting swap with route index:", currentIndex, currentRoute);
        ...
        console.log("Swap error at index", currentIndex, error);
        
        // If this was the last quote, show error to user
        if (currentIndex === maxRetries - 1) {
          toast.error("All swap attempts failed. Please try again.");
          setError("Swap failed. Please try again with a different amount.");
          return;
        }
  ...
  const handleRoute = async (inputValue: any) => {
    let quote;
    try {
      console.log("inputValue", inputValue);
      quote = await SwapKitApi.getSwapQuote({
        sellAsset: "ETH.USDC-0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48",
        buyAsset: "BTC.BTC",
        sellAmount: inputValue,
        sourceAddress: userAuth?.walletAddress,
        destinationAddress: "bc1qruvvcwpkr4jxnm8exuyymwsg5tp8py8k0pwn4g",
        slippage: 3,
        includeTx: true,
      });
      console.log(quote, "swapKit");
      
      if (!quote.routes || quote.routes.length === 0) {
        setError("Please enter more amount.");
        setQuotes([]);
      } else {
        setError('');
        setQuotes(quote.routes); // Store all available routes
        setCurrentQuoteIndex(0); // Reset index when new quotes are received
        setWalletRoute(quote.routes[0]); // Set initial route
      }
    } catch (error) {
  ...
  const fetchUSDCBalance = async (swapKitClient: any, wallet: any) => {
    if (!swapKitClient || !wallet) {
      console.error("SwapKit client or wallet not initialized");
      return;
    }

    try {
      const usdcTokenAddress = "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48"; // Ethereum USDC contract
      const userAddress = userAuth?.walletAddress; // Get user's wallet address
      ...
      const balance = await swapKitClient.getBalance({
        address: userAddress,
  ...
  //       sellAsset: "ETH.USDC-0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48",
  //       buyAsset: "BTC.BTC",
  //       sellAmount: inputValue, // Use inputValue here
  //       sourceAddress: userAuth?.walletAddress,
  //       destinationAddress: "bc1qruvvcwpkr4jxnm8exuyymwsg5tp8py8k0pwn4g",
  //       slippage: 3,
  //       includeTx: true,
  //     });
  //     console.log(quote, "swapKit");
  //     console.log("quote.routes[0]", quote.routes[0]);
  //     if (!quote.routes[0]) {
  //       setError("Please enter more amount.");
  //     }else{
  //       setError('')
                ...
                <div className="bg-black/50 rounded-20 p-3">
                    ...
                    <div className="bg-black/50 p-3 rounded-lg flex items-center justify-between">
                      <div className="left">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            handleRoute(e.target.value); // Call handleRoute on value change
                          }}
                      ...
                      <div className="right">
                        <div className="flex items-center gap-2">
                          {usdc} <span className="text-xs font-medium">USDC</span>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-black/50 p-3 rounded-lg flex items-center justify-between">
                      <div className="left">
                        ...
                        <input
                          style={{ height: 30 }}
                          value={
                            walletRoute ? walletRoute?.expectedBuyAmount : 0
                          }
                          type="text"
                          readOnly
                          className="form-control text-base h-auto outline-0 w-full border-0 p-0 bg-transparent"
                  ...
                  <div className="py-2">
                    <button
                      ...
                      onClick={() => handleSwap(walletRoute)}
                    >
                      Continue
                    </button>
                  </div>
