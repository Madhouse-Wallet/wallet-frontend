...
interface Web3AuthhProps {
  setWeb3AuthProviderC: (provider: any) => void;
  setUserInfo: (provider: any) => void;
  registerFn: (auth: any, provider: any) => void;
}
const Web3Authh: React.FC<Web3AuthhProps> = ({ setWeb3AuthProviderC, setUserInfo, registerFn }) => {
    ...
    const init = async () => {
      try {
        ...
        if (web3auth.connected) {
          ...
          setUserInfo(user)
          setWeb3AuthProviderC(web3auth.provider);
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const login = async () => {
    ...
    try {
      ...
      setWeb3AuthProviderC(web3authProvider)
      const user = await web3auth.getUserInfo();
      setUserInfo(user)
      registerFn(web3auth,user)
      setLoggedIn(true);
    } catch (error) {
      console.error(error);
    }
  ...
  const loggedInView = (
    <>
      <div style={{ "color": "green" }} className="flex-container">
        <div>
      ...
      <div id="console" style={{ whiteSpace: "pre-line", "color": "green" }}>
  ...
  return (
    <div style={{ "color": "green" }} className="">
      ...
      <div className="grid">{loggedIn ? (logoutView) : unloggedInView}</div>
